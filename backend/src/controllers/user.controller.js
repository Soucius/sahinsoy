...
export async function getAllUsers(_, res) {
    try {
        const users = await User.find().populate("user_role", "role_name").select("-user_password").sort({ createdAt: -1 });
...
export async function createUser(req, res) {
    try {
        ...
        const newUser = new User({
            user_username, user_email, user_phone, user_password,
        ...
        if (error.code === 11000) return res.status(400).json({ message: "Bu e-posta veya telefon numarası zaten kayıtlı." });
...
export async function forgotPassword(req, res) {
    try {
        ...
        if (!user) return res.status(404).json({ message: "Bu e-posta adresine ait kullanıcı bulunamadı." });
        ...
        await sendEmail({ to: user.user_email, subject: "Şahinsoy Perde - Şifre Sıfırlama Kodu", text: emailText });
...
export async function resetPassword(req, res) {
    try {
        ...
        const user = await User.findOne({ 
            user_email, resetPasswordOTP: otp, resetPasswordExpires: { $gt: Date.now() }
        });

        if (!user) return res.status(400).json({ message: "Geçersiz veya süresi dolmuş kod girdiniz." });
